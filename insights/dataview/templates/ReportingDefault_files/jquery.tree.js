		...
		return {
			...
			refresh : function (obj) {
				...
				_datastore.load(this.callback("beforedata",[false,this]),this,this.settings.data.opts,function(data) {
					...
					_datastore.parse(true, data,_this,_this.settings.data.opts,function(dom) {
						//str = _this.callback("onparse", [str, _this]); //lih get onparse working again
				                //_this.container.empty().append($("<ul class='ltr'>").html(str));
					        _this.container.empty().append(dom);
			...
			attach_events : function () {
				...
				this.container
			        ...
			        //Fixed by lih: We don't want to alter the behavior for all li tags.
			        // add .tree to the selector
    			       $(".tree li", this.container.get(0))
			        ...
			        //Fixed by lih: We don't want to alter the behavior for all anchor tags.
			        // add .tree to the selector
			       $(".tree a", this.container.get(0))
					.live("click", function (event) { // WHEN CLICK IS ON THE TEXT OR ICON
						...
						_this.callback('onsingleclk', [_this.get_node(event.target).get(0), _this]);
		  ...
		  load_datastore : function(obj, callback, do_not_open_branch){
		    var _this = this;
			...
			obj.children("ul:eq(0)").remove().end().append("<ul><li class='last'><a class='loading' href='#'><ins>&nbsp;</ins>" + (_this.settings.lang.loading || "Loading ...") + "</a></li></ul>");
		      if(do_not_open_branch == undefined)
			obj.removeClass("closed").addClass("open");
		      ...
		      _datastore.load(this.callback("beforedata",[obj,this]),this,this.settings.data.opts,function(data){
					...
					_datastore.parse(false,data,_this,_this.settings.data.opts,function(dom){
							   ...
							   // if(obj.children('ul:eq(0)').children('li').size() > 1) obj.children("ul").find('.loaading').parent().replaceWith(str); else
						           ...
						           //obj.children("ul:eq(0)").replaceWith($("<ul></ul>").append(dom));
							   obj.children("ul:eq(0)").replaceWith(dom);
		  ...
		  open_branch : function (obj, disable_animation, callback) {
		    ...
		    var obj = this.get_node(obj);
				...
				if(this.settings.data.async && obj.find("li").size() == 0) {
					...
					// 	});
					// });
				  this.load_datastore(obj, callback);
					return true;
				}
				else {
					if(!this.settings.data.async) {
			...
			close_branch : function (obj, disable_animation) {
				...
				if(this.selected && this.settings.ui.selected_parent_close !== false && obj.children("ul:eq(0)").find("a.clicked").size() > 0) {
				  obj.find("li:has(a.clicked)").each(function() {
				        ...
				        //commented out by lih.
				         //if(this.settings.ui.selected_parent_close == "select_parent" && obj.children("a.clicked").size() == 0){
	...
	$.extend($.tree.datastores, {
		...
		"json" : function () {
			return {
			  ...
			  parse_helper : function(data, tree, opts, $parent_dom){
			    //var pos = sb.length;
			    var sb = [];
			    var pos = 0;
				...
				$parent_dom.append(this.parse_helper(data[i], tree, opts, $parent_dom));
			      }
			    }
			    if(!data || !data.data) {
			      // if(callback){
				...
				if(data.data[tree.settings.languages[i]] && data.data[tree.settings.languages[i]].icon) {
				  ...
				  sb[pos++] = "<ins ";
				  if(data.data[tree.settings.languages[i]].icon.indexOf("/") == -1){
				    sb[pos++] = " class='";
				    sb[pos++] = data.data[tree.settings.languages[i]].icon;
				    sb[pos++] = "' ";
				  }else{
				    sb[pos++] = " style='background-image:url(\"";
				    sb[pos++] = data.data[tree.settings.languages[i]].icon;
				    sb[pos++] = "\");' ";
				  }
				  sb[pos++] = ">&nbsp;</ins>";
				...
				if((typeof data.data[tree.settings.languages[i]].title).toLowerCase() != "undefined"){
				  sb[pos++] = data.data[tree.settings.languages[i]].title;
				}else{
				  sb[pos++] = data.data[tree.settings.languages[i]];
				}
				sb[pos++] = "</a>";
			      }
			    }
			      ...
			      attr["href"] = "";
			      attr["style"] = "";
			      // hack in by lih to initialize check box state
			      if( data.check_box_state != undefined)
				attr["class"] = data.check_box_state;
			      else
				attr["class"] = "";
				...
				sb[pos++] = "<ins ";
				if(data.data.icon.indexOf("/") == -1){
				  sb[pos++] = " class='";
				  sb[pos++] = data.data.icon;
				  sb[pos++] = "' ";
				}else{
				  sb[pos++] = " style='background-image:url(\"";
				  sb[pos++] = data.data.icon;
				...
				sb[pos++] = "<ins>&nbsp;</ins>";
			      ...
			      if((typeof data.data.title).toLowerCase() != "undefined"){
				sb[pos++] = data.data.title;
			      }else{
				sb[pos++] = data.data;
			      }
			      sb[pos++] = "</a>";
			    ...
			    var html_str = sb.join('');
			    var $curr_dom = $(html_str);

			    if(data.children && data.children.length) {
			      var $new_parent = $('<ul></ul>');
			      for(var i = 0; i < data.children.length; i++) {
				var $new_dom = this.parse_helper(data.children[i], tree, opts, $new_parent);
				if($new_dom!=null)
				  $new_parent.append($new_dom);
			      }
			      $curr_dom.append($new_parent);
			    }
			    return $curr_dom;
			  }, // parse_helper

			  parse	: function(is_during_construction, data, tree, opts, callback) {
			    if(Object.prototype.toString.apply(data) === "[object Array]") {
			      if(is_during_construction)
				var $parent_dom = $("<ul class='ltr'></ul>");
			      else
				var $parent_dom = $("<ul></ul>");
			      this.parse_helper(data, tree, opts, $parent_dom);
			      if(callback){
				callback.call(null, $parent_dom);
			      }
			      return;
			    }
