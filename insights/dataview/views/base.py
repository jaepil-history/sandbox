...
class BaseView(tornado.web.RequestHandler):
    ...
    def initialize(self):
        ...
        self.now = datetime.utcnow().today().date()
    ...
    def get_current_user(self):
        ...
        if getenv('DATAVIEW_ENV_HTTP_TEST', None) == 'test':
            return 1
    ...
    def finish(self, chunk = None):
        ...
        if self.session is not None and self.session._delete_cookie:
            self.clear_cookie('dataview_session_id')
        elif self.session is not None:
            self.session.refresh() # advance expiry time and save session
            self.set_secure_cookie('dataview_session_id', self.session.session_id, expires_days=None,
    ...
    def _create_session(self):
        session_id = self.get_secure_cookie('dataview_session_id')
