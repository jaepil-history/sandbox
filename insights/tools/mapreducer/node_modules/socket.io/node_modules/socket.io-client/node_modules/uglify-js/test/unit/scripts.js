...
function getTester(script) {
	return function(test) {
		...
		// Stats if the noncompressdata is larger or same size as the compressed data
		test.ok(content.length >= outputCompress.length);

		// Stats that a recompress gives the same result
		var outputReCompress = compress(content);
		test.equal(outputCompress, outputReCompress);

		// Stats if the compressed output is what is expected
