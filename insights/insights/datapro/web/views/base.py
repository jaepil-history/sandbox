...
from insights.datapro.web.models import unread_model
from insights.datapro.web.libs.session import MongoDBSession
from insights.datapro.web.template import render as jinja_render
...
class BaseView(tornado.web.RequestHandler):
    ...
    def get_current_user(self):
        ...
        if getenv('DATAPRO_ENV_HTTP_TEST', None) == 'test':
            return 1
    ...
    def finish(self, chunk = None):
        ...
        if self.session is not None and self.session._delete_cookie:
            self.clear_cookie('datapro_session_id')
        elif self.session is not None:
            self.session.refresh() # advance expiry time and save session
            self.set_secure_cookie('datapro_session_id', self.session.session_id, expires_days=None,
    ...
    def _create_session(self):
        session_id = self.get_secure_cookie('datapro_session_id')
