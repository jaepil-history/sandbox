...
{
	using BufferQueue = Queue<Buffer>;
	...
	public class Link
	{
		...
		private Threading.ThreadSafeObject<BufferQueue> writeBuffers_ = new Threading.ThreadSafeObject<BufferQueue>();
		private Threading.ThreadSafeObject<MemoryStream> readStream_ = new Threading.ThreadSafeObject<MemoryStream>();
		...
		public Link(Socket handle, IOEngine engine)
		{
			...
			if (handle_ == null)
			{
				Logging.Log.Error("[Link::Link()] Invalid socket handle");
				throw new System.Exception();
			}

			if (engine_ == null)
			{
				Logging.Log.Error("[Link::Link()] engine_ == null");
				throw new System.Exception();
			}
		...
		public bool Send(byte[] data)
		{
			if (data == null)
			{
				Logging.Log.Error("[Link.Send()] data == null.");
				return false;
			}

			if (data.Length == 0)
			{
				Logging.Log.Error("[Link.Send()] data.Length == 0.");
				return false;
			}
		...
		{
			BufferQueue bufferQueue = LockWriteQueue();
			try
			{
		...
		private bool InternalSend(Buffer buffer)
		{
			if (buffer == null)
			{
				Logging.Log.Error("[Link.InternalSend] buffer == NULL.");
				return false;
			}

			if (buffer.IsEmpty())
			{
				Logging.Log.Error("[Link.InternalSend] buffer is empty.");
				return false;
			}
		...
		{
			BufferQueue writeBuffers = writeBuffers_.Lock();
			try
			{
				writeBuffers.Clear();
			}
